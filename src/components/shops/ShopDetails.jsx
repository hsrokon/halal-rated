...
const ShopDetails = () => {
  ...
  return (
    <div className="w-11/12 lg:w-10/12 xl:w-8/12  mx-auto mb-20">
      {/* Image */}
      <div className="">
        <figure>
          <img
            src={shop.photoURL}
            alt={shop.placeName}
            className="w-full h-40 md:h-[22rem] lg:h-[25rem] xl:h-[33rem] object-cover"
          />
        </figure>
        <div className="">
          {/*name */}
          <div className="flex items-center justify-between bg-secondary/80 my-2 lg:my-4 xl:my-6 py-0.5 md:py-1 lg:py-2 pl-4">
            <h1 className="text-lg lg:text-3xl font-bold text-white">{shop.placeName}</h1>
            {shop.halalCertified ? (
          ...
          <div className="flex flex-col text-sm md:flex-row md:justify-between md:items-center">
            ...
            <div className="flex items-center gap-2 text-gray-700">
              <MapPin className="text-primary size-5 md:size-6 lg:size-8" />
            ...
            <div className="flex flex-wrap gap-2">
              {shop.selectedTags.map((tag, i) => (
                <span key={i} className="badge badge-outline flex text-sm items-center gap-1">
                  <Tag className="text-primary size-3 md:size-4 lg:size-5 xl:size-6" /> {tag}
                </span>
              ))}
          </div>
          ...
          <div className="flex flex-col md:flex-row md:justify-between md:items-center mt-4">
            <p className="text-sm text-base-content hover:underline">
              ...
              <span className="text-accent font-semibold">{user && user.displayName}</span></p>
            <p className="text-xs md:tex-sm text-gray-500 hover:text-base-content hover:underline">Enlisted on : <span> </span> {enlistedDate}</p>
          </div>
        </div>
